...
export const handleListCreatives = async (args?: Record<string, unknown>): Promise<McpResponse> => {
	...
	mcpFileLog("[MCP TOOL INVOCATION] handleListCreatives called with args:", args);
	...
	mcpFileLog("[MCP TOOL INVOCATION] handleListCreatives parsedArgs:", parsedArgs);
	...
	if (parsedArgs.advertiserIds.length === 0 && selectedAdvertiserId) {
		...
		mcpFileLog("[MCP TOOL INVOCATION] handleListCreatives using selectedAdvertiserId:", selectedAdvertiserId);
	...
	mcpFileLog(`[MCP TOOL INVOCATION] handleListCreatives response: ${items.length} creatives, nextPageToken: ${nextPageToken}`);
